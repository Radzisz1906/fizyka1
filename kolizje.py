...
def dodaj(L, H, R, vGen, atomy):


    i = len(atomy)
    while i != 0:
        xx = random.uniform(0.0 + R, L - 2 * R)
        yy = random.uniform(0.0 + R, H - 2 * R)
        i = len(atomy)
        at = Atom(xx, yy, random.uniform(-1.0 * vGen, vGen), random.uniform(-1.0 * vGen, vGen), s, (0, 0, 255))
        print(i)
        for j in range(len(atomy)):
            if (at.dystans(atomy[j])) <= s + tol + 4:
                break
...
time = 0
lambdy = []
czerfony = Atom(0, 0, random.uniform(-1.0 * vGen, vGen), random.uniform(-1.0 * vGen, vGen), s, (255, 0, 0))
...
def main():
    global etaH,etaL,R,s,H, L, vGen,tol,M, atomy, lamby, time
